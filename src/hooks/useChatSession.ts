...
export const useChatSession = (
  ...
  const handleSendPress = async (message: MessageType.PartialText) => {
    ...
    // Inject curriculum context if curriculum is active
    let finalSystemMessages = systemMessages;
    if (curriculumStore.isActive && curriculumStore.hasFullSelection) {
      const curriculumContext = curriculumStore.getContextPrompt();
      if (curriculumContext) {
        // Add curriculum context as a system message or prepend to existing
        finalSystemMessages = [
          ...systemMessages,
          {role: 'system' as const, content: curriculumContext},
        ];
      }
    }
    ...
    const {cleanCompletionParams, messageInfo} = await prepareCompletion({
      imageUris: imageUris || [],
      message,
      systemMessages: finalSystemMessages,
      contextId,
      assistant,
